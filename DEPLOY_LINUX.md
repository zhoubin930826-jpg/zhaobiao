...
```bash
scp target/zhaobiao-admin-0.0.6-SNAPSHOT.jar your_user@your_server_ip:/opt/zhaobiao/app/
```

示例：

```bash
scp target/zhaobiao-admin-0.0.6-SNAPSHOT.jar ubuntu@123.123.123.123:/opt/zhaobiao/app/
```
...
source app.env
set +a
nohup java -jar zhaobiao-admin-0.0.6-SNAPSHOT.jar > app.log 2>&1 &
```
...
ExecStart=/usr/bin/java -jar /opt/zhaobiao/app/zhaobiao-admin-0.0.6-SNAPSHOT.jar
