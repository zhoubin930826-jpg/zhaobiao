...
module.exports = {
    ...
    devServer: {
        publicPath: Setting.publicPath,
        // 开发环境：将浏览器请求 `/api/*` 转发到后端（与 axios `apiBaseURL: '/api'` 配合）。
        // 不设环境变量时默认 `https://xiazhiyong.vip`，与门户 `front/`（VITE_PROXY_TARGET）默认一致。
        // 本地后端示例（Spring Boot 默认端口见项目根 application.yml）：
        //   CMD:   set VUE_APP_PROXY_TARGET=http://127.0.0.1:8080&& npm run dev
        //   PowerShell: $env:VUE_APP_PROXY_TARGET='http://127.0.0.1:8080'; npm run dev
        proxy: {
            '/api': {
