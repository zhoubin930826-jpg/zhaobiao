...
function filterMenu (menuList, access, lastList) {
    ...
    access.forEach((item, index) => {
        ...
        treeItem.custom = 'i-icon-demo i-icon-demo-dashboard'
        treeItem.path = item.fe_path
        treeItem.children = []
        treeItem.icon = `md-locate`
        if (item.child) {
...
export default {
    ...
    getters: {
        ...
        filterSider (state, getters, rootState) {
            ...
            if (authTree && authTree.length) {
                return filterMenu(state.sider, authTree, []);
            } else {
